...
class ICaRL(nn.Module):
  def __init__(self, feature_size, n_classes,
    ...
    self.optimizer, self.scheduler = utils.getOptimizerScheduler(
        self.LR, self.MOMENTUM, self.WEIGHT_DECAY, self.MILESTONES, self.GAMMA, self.parameters())
    ...
    self.class_loss, self.dist_loss, self.task_loss = self.build_loss(
        class_loss_criterion, dist_loss_criterion, loss_rebalancing, lambda0=lambda0)
  ...
  def FCC_classify(self, images):
    _, preds = torch.max(torch.softmax(
        self.net(images), dim=1), dim=1, keepdim=False)
    return preds
  ...
  def construct_exemplar_set(self, tensors, m, label):
    ...
    if self.herding:
      ...
      loader = DataLoader(tensors, batch_size=self.BATCH_SIZE,
      ...
      summon = torch.zeros(1, features_s.size()[1]).to(
          self.DEVICE)  # (1,num_features)
      for k in range(1, (m + 1)):
          # second addend, features in the exemplar set
          S = torch.cat([summon]*features_s.size()[0])
          results = pd.DataFrame((class_mean-(1/k)*(features_s + S)).pow(
              2).sum(1).cpu(), columns=['result']).sort_values('result')
          ...
          for i in range(results.shape[0]):
            ...
            if exemplar_k_index not in exemplar_set_indices:
              ...
              # index of the img on the real dataset
              exemplar_k_index = tensors[index][0]
          ...
          # feature_extractor(exemplar_k.to(self.DEVICE))
          phi = feature_extractor(exemplar_k.to(self.DEVICE))
          summon += phi  # update sum of features
    else:
      tensors_size = len(tensors)
      # random sample without replacement k exemplars
      unique_random_indexes = random.sample(range(0, tensors_size), m)
      i = 0
      for k in range(1, (m + 1)):
    ...
    # update exemplar sets with the updated exemplars images
  ...
  def train_new_task(self, dataset, train_dataset_big, new_classes):
    ...
    for epoch in range(self.NUM_EPOCHS):
        print("NUM_EPOCHS: ", epoch+1, "/", self.NUM_EPOCHS)
        for _, images, labels in loader:
            # Bring data over the device of choice
            images = images.to(self.DEVICE)
            labels = labels.to(self.DEVICE)
        ...
        scheduler.step()
        print("LOSS: ", loss.item(), 'class loss', class_loss, 'dist loss', 'no dist loss')
      
    print('End task net training')
  ...
  def build_loss(self, class_loss_criterion, dist_loss_criterion, rebalancing=None, lambda0=1):
    ...
    def task_loss(outputs, labels, row_start=None, row_end=None, col_start=None, col_end=None):
      return task_loss_func(outputs, labels, task_label_mapping=True, row_start=row_start, row_end=row_end, col_start=col_start, col_end=col_end)
    
    return class_loss, dist_loss, task_loss

  def bce_class_loss(self, outputs, labels, task_label_mapping=False, row_start=None, row_end=None, col_start=None, col_end=None):
    return self.bce_loss(outputs, labels, task_label_mapping=task_label_mapping, encode=True, row_start=row_start, row_end=row_end, col_start=col_start, col_end=col_end)
  ...
  def ce_class_loss(self, outputs, labels, task_label_mapping=False, row_start=None, row_end=None, col_start=None, col_end=None):
  ...
  def l2_class_loss(self, outputs, labels, task_label_mapping=False, row_start=None, row_end=None, col_start=None, col_end=None):
    return self.l2_loss(outputs, labels, task_label_mapping=task_label_mapping, encode=True, row_start=row_start, row_end=row_end, col_start=col_start, col_end=col_end)
  ...
  def bce_loss(self, outputs, labels, task_label_mapping=False, encode=False, row_start=None, row_end=None, col_start=None, col_end=None):
    criterion = nn.BCEWithLogitsLoss(reduction = 'mean')

    if encode:
      labels = self._one_hot_encode(labels, self.n_classes, self.reverse_index, task_label_mapping=task_label_mapping, device=self.DEVICE)
      labels = labels.type_as(outputs)

    return criterion(outputs[row_start:row_end, col_start:col_end], labels[row_start:row_end, col_start:col_end])


  def ce_loss(self, outputs, labels, task_label_mapping=False, decode=False, row_start=None, row_end=None, col_start=None, col_end=None):
  ...
  def l2_loss(self, outputs, labels, task_label_mapping=False, encode=False, row_start=None, row_end=None, col_start=None, col_end=None):
  ...
  def map_to_outputs(self, labels, reverse_index, task_label_mapping=False):
    if not task_label_mapping:
